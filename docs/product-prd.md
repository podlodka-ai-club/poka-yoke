...
- Пользователи этих сегментов, уже работающие в Pi (pi.dev); фактический контекст и доступность Pi-интеграции unknown.
...
**Scope.**
- Прикладная двухнедельная проверка в контексте Pi; коммуникация и продуктовые результаты на русском языке.
...
- Повторные или связанные claims с сопоставлением stateless и memory-условий, включая correction/reset.
- Оценка со студентами и research staff, по возможности с comparator в виде универсального research copilot.
...
- Evaluation: какие comparator и stateless baseline, gold labels, sample, минимальный эффект, stopping rule и критерии сравнения с generic research copilot принимаются?
- Pi integration: какая фактическая целевая поверхность и user flow в pi.dev доступны для пилота и какие ограничения означают, что product fit ещё не проверен?
- Memory-engine spike: какой repeated/related-claim сценарий и порог эффекта докажут ценность memory, и как принимаются staleness, contamination и correction/reset failures?
...
- Ограничения интеграции с Pi могут не позволить воспроизвести реальный workflow и исказить результат.
...
- Evaluation: заранее описать сопоставление узкого режима с generic research copilot и stateless baseline на повторных или связанных claims; определить sample, gold labels, минимальные effect sizes и stopping rule.
- Memory-engine spike: установить, даёт ли evidence reuse, correction/reset измеримую пользу и выявить stale/contamination failures; определить memory value как task outcome, а не как объём сохранённых данных или число recall.
- Pi integration: проверить реальный user flow в Pi (pi.dev), целевую поверхность и возможность провести пилот без изменения контекста задачи; до этого нельзя заявлять product fit.
- User validation: привлечь студентов и research staff, наблюдать реальные RU/EN claims, измерить input burden, completion, comprehension, trust, repeat intent и fit против generic copilot; зафиксировать различия сегментов.
...
### Evidence trace
- [verified: V1 | shared context] Pi — это pi.dev и software platform → Pi является целевой продуктовой средой; конкретный путь интеграции и fit не подтверждены.
...
- [unknown: U3 | evidence gap] Gold labels, judging rubric, comparator, sample, effect thresholds и stopping rules не установлены → требуется evaluation/rubric decision.
- [unknown: U4 | evidence gap] Pi integration feasibility и representative user flow не установлены → требуется Pi validation.
- [unknown: U5 | evidence gap] Incremental task value, freshness и contamination behavior memory engine не установлены → требуется memory spike.
...
- Фрейминг не до конца согласован с исследованием Pi: пользователь уже подтвердил pi.dev, а старый вариант допускал интерпретацию Raspberry Pi. Нужно окончательно убрать аппаратную трактовку и описать последствия software runtime для демо.
...
- Альтернативы не получили единого scoring framework и явных причин отбора/отказа; benchmark-demo — скорее evaluation-режим, общий для других вариантов, чем самостоятельный продуктовый выбор.
...
- FEVER и SciFact могут быть загрязнены обучающими данными моделей; сравнение stateless и memory на них рискует измерять memorization/contamination, а не пользу памяти. Нужны held-out claims и контроль утечек.
...
- Положительная сторона: фрейминг честно перечисляет unknowns и не выдумывает numeric targets. Но прозрачный список неизвестных ещё не заменяет закрытие тех, что блокируют выбор MVP.

### Blocking gaps
- Зафиксировать, что Pi — программная платформа pi.dev, и определить runtime/package/demo consequences.
...
- Провести короткий hands-on spike memory-кандидатов и подтвердить typed records, passages, relations, versioning, correction/reset, Pi integration и локальный запуск до выбора движка.
...
- **Claim:** Бриф содержит ссылки на воркшопы по памяти агентов и на mem0, Cognee, xmemory, Supermemory, Graphiti и Letta; наличие ссылок подтверждено, пригодность для данного MVP требует отдельной проверки.
  - **Status:** verified
  - **Source:** Notion-бриф: https://app.notion.com/p/Hacker-Sprint-2-3a12db4c860e80928153c5d511b367ed?session_sync_attempted=1
- **Claim:** Pi.dev позиционируется как терминальный coding agent с расширениями, skills, пакетами и SDK; это программная платформа, а не Raspberry Pi.
  - **Status:** verified
...
- **Claim:** SciFact оценивает поиск научных доказательств, классификацию SUPPORT/CONTRADICT и sentence-level rationale для научного утверждения.
  - **Status:** verified
  - **Source:** Статья SciFact: https://aclanthology.org/2020.emnlp-main.609/; репозиторий: https://github.com/allenai/scifact
...
- **Claim:** FEVER и SciFact дают полезные evaluation-паттерны, но их схемы нельзя безоговорочно считать готовой политикой для русского/английского продукта: нужно отдельно определить UNKNOWN, смешанные свидетельства и качество исследования.
  - **Status:** assumption
  - **Source:** Сопоставление спецификаций FEVER и SciFact по указанным первичным источникам
...
- **Claim:** Cognee документирует построение и использование knowledge graph/knowledge engine для AI-приложений, однако пригодность его схемы для passages, contradictory evidence и audit-ready verdicts в двухнедельном MVP неизвестна.
  - **Status:** verified
  - **Source:** Cognee documentation: https://docs.cognee.ai/core-concepts/overview
...
- **Claim:** Ни один из шести кандидатов памяти по доступным публичным материалам не доказан как готовый end-to-end слой для typed claim, source passage, support/refute/UNKNOWN, temporal correction, inspect/reset и evaluation isolation одновременно.
  - **Status:** unknown
  - **Source:** Сравнительный review публичных материалов mem0, Cognee, xmemory, Supermemory, Graphiti и Letta; требуется hands-on spike
- **Claim:** Vector-only memory сама по себе не гарантирует явные отношения между claim, source, passage, contradiction и версией; это проектный вывод, а не доказанное свойство каждого конкретного вендора.
  - **Status:** assumption
  - **Source:** Проектная гипотеза, подлежащая проверке на выбранном memory engine
...
- Собрать bilingual held-out evaluation set с русскими и английскими claims, золотыми evidence passages и ручными метками; целевой размер и labels неизвестны.
- Определить protocol stateless-vs-memory: held-out claims, запрет утечки train/evidence, passage recall, verdict accuracy, provenance completeness, latency, новые загрузки и correction rate.
- Провести короткий hands-on spike для шести memory-кандидатов: хранение typed records, passage provenance, relations/contradictions, versioning, inspect, correction, reset, Pi.dev integration, локальный запуск, стоимость и усилия.
- Проверить, как сохранять оригинальную англоязычную цитату и русское резюме без потери смысла и как обрабатывать cross-language retrieval.
...
- Необходима проверка доверия к Pi.dev как runtime и способа упаковки расширения: официальные docs подтверждают платформу/SDK, но не готовую научную интеграцию.
...
- **B-structured-claim-evidence-ledger:** Ограниченный claim-level verifier с явными сущностями claim, source, passage, evidence relation, verdict, run и correction; память переиспользуется между проверками.
  - **Pros:** Лучший баланс между темой памяти, проверяемостью и двухнедельным scope.; Каждую связь можно показать пользователю и исправить.; Естественно поддерживает stateless-vs-memory сравнение и UNKNOWN.
  - **Cons:** Нужно выбрать ограниченный корпус и не обещать полноту научного поиска.; Схема contradiction/versioning и bilingual retrieval требует явных тестов.; Не гарантирует истинность только за счёт хранения evidence.
...
- Не обучение и не файнтюнинг моделей: используются существующие возможности LLM/pi.dev и готовые memory-решения.
...
- Движок памяти НЕ ВЫБРАН (TBD): пригодность mem0/Cognee/xmemory/supermemory/Graphiti/Letta для сценария с коррекцией, сбросом и провенансом не исследована; выбор — предмет следующего этапа исследования.
...
### Problem statements
- Студенты и научные сотрудники, работающие с русско- и англоязычной научной литературой, проверяют научные утверждения вручную по первоисточникам, а сделанные выводы теряются между сессиями: у них нет агента с долговременной памятью, который накапливал бы результаты проверок и переиспользовал их при новых утверждениях.
- За две недели хакатона «Hacker Sprint #2: Агент, который помнит» нужно довести до прикладного результата агента на платформе pi.dev, решающего одну узкую задачу проверки утверждений так, чтобы им можно было пользоваться лично или на работе уже после спринта, а не только в демо.
- Студентам и сотрудникам, работающим с научной литературой и исследованиями, нужен более точный способ получать от Pi (pi.dev) помощь с учётом накопленного исследовательского контекста, а не начинать каждое взаимодействие без проверенной памяти.
...
- Бриф содержит ссылки на воркшопы по управлению памятью LLM-агентов и проекты mem0, Cognee, xmemory, supermemory, Graphiti, Letta; существование ссылок верифицировано, техническая пригодность этих систем для нашей задачи — предмет следующего этапа исследования.
- Платформа агента зафиксирована как pi.dev (явное подтверждение пользователя); точная техническая роль pi.dev в архитектуре решения пока не описана.
...
- Pi в данном запросе означает pi.dev.
- Brief содержит ссылки на workshops по memory management и материалы/проекты mem0, Cognee, xmemory, supermemory, Graphiti и Letta; наличие ссылок подтверждено, пригодность для продукта ещё не исследована.
...
- Команда разработки спринта — реализует агента на pi.dev за две недели хакатона.
- Организаторы и жюри Hacker Sprint #2 — оценивают результат; их критерии и формат демо пока неизвестны.
- Студенты, работающие с научной литературой и исследованиями.
- Сотрудники исследовательских групп и научные сотрудники.
- Организаторы и участники Hacker Sprint #2.
- Заказчик продукта, которому нужен точный русский PRD и проверяемая формулировка ценности.
...
- Базовая платформа задана: pi.dev; выбор библиотеки памяти из списка брифа (mem0, Cognee, xmemory, supermemory, Graphiti, Letta) не предрешён и требует сравнительного исследования.
...
- Инфраструктурный бюджет (локальное vs облачное хранение памяти, платные внешние API) не зафиксирован и должен быть определён до выбора корпуса и механизма памяти.
...
- Нельзя считать любую связанную memory-систему подходящей или доказавшей пользу до отдельного исследования.
- Нужно явно учитывать Pi=pi.dev и не подменять его другим продуктом.
...
- Оценка качества: на каких размеченных данных строим метрики (SciFact, BioASQ, HealthFC или собственная разметка), сколько меток — минимум для доверия, и с чем сравниваем baseline (LLM без памяти и без retrieval)?
...
- Как Pi (pi.dev) участвует в пользовательском потоке и какие его реальные возможности/ограничения нужно подтвердить?
- Какие из перечисленных memory-материалов и проектов технически пригодны для целевого сценария, и по каким критериям сравнения?
...
- **Claim:** Бриф содержит ссылки на воркшопы по управлению памятью агентов и проекты mem0, Cognee, xmemory, supermemory, Graphiti, Letta; существование ссылок подтверждено, техническая пригодность проектов не оценена.
  - **Status:** verified
  - **Source:** Notion-бриф (перечень ссылок)
...
- **Claim:** Под «Pi» понимается платформа pi.dev.
  - **Status:** verified
...
- **Claim:** Целевой язык коммуникации — русский; исходные исследовательские материалы могут быть на русском и английском.
  - **Status:** verified
  - **Source:** Прямое подтверждение пользователя
- **Claim:** Предыдущий раунд уточнения признан пользователем слишком слабым; запрошены более точный фрейминг и PRD на русском языке.
  - **Status:** verified
  - **Source:** Запрос пользователя (текущая итерация)
...
- **Claim:** Пригодность mem0/Cognee/xmemory/supermemory/Graphiti/Letta для данной задачи и их сравнительные преимущества неизвестны.
  - **Status:** unknown
  - **Source:** Ссылки есть в брифе; оценка не проводилась — предмет этапа исследования
...
- **Claim:** Notion brief имеет заголовок «Hacker Sprint #2: Агент, который помнит».
  - **Status:** verified
  - **Source:** Notion brief (предоставленная пользователем; подтверждено browser access)
- **Claim:** Видимое введение brief описывает двухнедельный спринт по созданию прикладной реализации для личного или рабочего использования.
  - **Status:** verified
  - **Source:** Notion brief (видимое intro; подтверждено browser access)
- **Claim:** Brief содержит ссылки на memory-management workshops и mem0, Cognee, xmemory, supermemory, Graphiti и Letta.
  - **Status:** verified
  - **Source:** Notion brief (существование ссылок подтверждено; пригодность не подтверждена)
- **Claim:** Основные пользователи — студенты и research staff, работающие с научной литературой и исследованиями.
  - **Status:** verified
  - **Source:** Явное подтверждение пользователя
- **Claim:** Pi означает pi.dev.
  - **Status:** verified
  - **Source:** Явное подтверждение пользователя
- **Claim:** Коммуникация агента должна быть на русском, а исходные материалы могут быть русскими и английскими.
  - **Status:** verified
  - **Source:** Явное подтверждение пользователя
- **Claim:** Предложенный продукт улучшит качество или эффективность исследовательской работы.
  - **Status:** unknown
  - **Source:** n/a; требует пользовательской проверки и исследования
- **Claim:** Перечисленные memory-системы подходят для целевого сценария.
  - **Status:** unknown
  - **Source:** n/a; требует отдельного evidence stage
- **Claim:** Можно завершить и доказать ценность решения в пределах двухнедельного спринта.
  - **Status:** unknown
  - **Source:** n/a; зависит от выбранного сценария и критериев успеха
